...
/// <summary>
/// A highly optimized dictionary for 2-byte unmanaged keys (e.g., ushort, char).
/// It uses a bitset and a value array ("flat") strategy for dense data and a custom hash table for sparse data.
/// This class is not thread-safe.
/// </summary>
public class TwoBytesKeyMap<K, V> : IDictionary<K, V>, IReadOnlyDictionary<K, V> where K : unmanaged{
    ...
    public TwoBytesKeyMap(IDictionary<K, V> dictionary) : this(dictionary?.Count ?? 0)
    {
        ArgumentNullException.ThrowIfNull(dictionary);
        AddRange(dictionary);
    }

    public TwoBytesKeyMap(IEnumerable<KeyValuePair<K, V>> collection) : this((collection as ICollection<KeyValuePair<K, V>>)?.Count ?? 0)
    {
    ...
    public int Count => IsFlatStrategy ?
                            _flatCount :
                            _loSize + _hiSize;
    ...
    private bool TryInsert(K key, V value, bool overwrite)
    {
        while( true )
        {
            ...
            else if( _entries!.Length <= _loSize + _hiSize )
            {
                ...
                if( flatStrategyThreshold < newSize && _entries.Length < flatStrategyThreshold ) newSize = flatStrategyThreshold;
                Resize(newSize);
                if( IsFlatStrategy ) continue; // Retry with the new flat strategy
            }
    ...
    public void Clear()
    {
        ...
        if( IsFlatStrategy )
        {
            ...
            if( RuntimeHelpers.IsReferenceOrContainsReferences<V>() ) Array.Clear(_flatValues!, 0, _flatValues!.Length);
        }
        else
        {
            if( _buckets != null ) Array.Clear(_buckets,                                                              0, _buckets.Length);
            if( RuntimeHelpers.IsReferenceOrContainsReferences<V>() && _loSize + _hiSize > 0 ) Array.Clear(_entries!, 0, _entries!.Length);
            _loSize = 0;
            _hiSize = 0;
        }
    }
    ...
    {
        var kv = key;
        if( IsFlatStrategy ) return (_nulls![toUshort(kv) >> 6] & 1UL << toUshort(kv)) != 0;
        ...
        {
            if( toUshort(_entries![i].key) == kUshort ) return true;

            if( _loSize <= i ) return false; //terminal entity
            i = _links![i];
    ...
    public bool ContainsValue(V value)
    {
        if( IsFlatStrategy )
        {
            ...
            {
                var bits = _nulls![i];
                if( bits == 0 ) continue; // Skip empty 64-bit bits
    ...
    private void Resize(int newSize)
    {
        ...
        if( IsFlatStrategy )
        {
            if( flatStrategyThreshold < newSize ) return;
            var oldNulls      = _nulls!;
            var oldFlatValues = _flatValues!;

            Initialize(newSize);
            for( var token = -1; (token = next1(token, oldNulls)) != -1; ) { Copy(Unsafe.As<int, K>(ref token), oldFlatValues[token]); }
        ...
        for( var i = 0; i                              < oldLoSize; i++ ) Copy(oldEntries![i].key,        oldEntries[i].value);
        for( var i = oldEntries!.Length - oldHiSize; i < oldEntries.Length; i++ ) Copy(oldEntries[i].key, oldEntries[i].value);
    }
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    ...
    public bool Remove(K key)
    {
        ...
        if( IsFlatStrategy )
        {
            ...
            var     t       = bitword;
            if( t == (bitword &= ~(1UL << k)) ) return false;

            if( RuntimeHelpers.IsReferenceOrContainsReferences<V>() ) _flatValues![k] = default!;
            _flatCount--;
            _version++;
            return true;
        }
        ...
        {
            if( toUshort(removeEntry.key) != kUshort ) return false;
        ...
        if( toUshort(removeEntry.key) == kUshort ) bucket = (ushort)(link + 1);
        else
        {
            var     last      = removeIndex;
            ref var lastEntry = ref removeEntry;

            if( toUshort((removeEntry = ref _entries![removeIndex = link]).key) == kUshort ) // The key is found at the second node
                if( removeIndex < _loSize ) link = _links[removeIndex];                      // 'SecondNode' is in 'lo Region', relink to bypasses 'SecondNode'
                else
                {
            ...
            else if( _loSize <= removeIndex ) return false;
            else
                for( var collisions = 0;; )
                {
                    ...
                    if( toUshort((removeEntry = ref _entries![removeIndex = link = ref _links![last = removeIndex]]).key) == kUshort )
                    {
                        if( removeIndex < _loSize ) link = _links[removeIndex];
                        else
                        {
                            removeEntry = lastEntry;
                            removeEntry = ref lastEntry;
                    ...
                    if( _loSize     <= removeIndex ) return false;
                    if( _loSize + 1 < collisions++ ) throw new InvalidOperationException("Concurrent operations not supported.");
                }
        }
    ...
    public struct Enumerator : IEnumerator<KeyValuePair<K, V>>{
        private readonly TwoBytesKeyMap<K, V> _map;
        private          int                  _version;
        private          int                  _index; // For flat: bit index; for hash: index into _entries
        private          KeyValuePair<K, V>   _current;

        internal Enumerator(TwoBytesKeyMap<K, V> map)
        {
            _map     = map;
        ...
        public bool MoveNext()
        {
            ...
            if( _map.IsFlatStrategy )
            {
                ...
                if( nextBit == -1 )
                {
                    _index = 0x10000; // Beyond valid range
                    return false;
                }
            ...
            if( _index == _map._loSize )
            {
                ...
                _index = _map._entries.Length - _map._hiSize;
            }
...
#region Flat Strategy Helpers
    ...
    protected static int next1(int bit, ulong[] nulls)
    {
        ...
        var i     = ++bit;
        var index = i >> 6;
        if( index >= nulls.Length ) return -1;
...
#region Dictionary Properties
    ...
    {
        ArgumentOutOfRangeException.ThrowIfNegative(capacity);
        var currentCapacity = _entries?.Length ?? (IsFlatStrategy ?
                                                       VALUES_SIZE :
                                                       0);
        if( currentCapacity >= capacity ) return currentCapacity;
        _version++;
        if( _buckets == null && !IsFlatStrategy ) return Initialize(capacity);
    ...
    public void TrimExcess()
    {
        ...
        if( IsFlatStrategy )
        {
            if( count <= flatStrategyThreshold ) Resize(GetPowerOfTwo(count));
        }
...
#region Collection Wrappers
    ...
    public sealed class KeyCollection : ICollection<K>, IReadOnlyCollection<K>{
        private readonly TwoBytesKeyMap<K, V> map;
        public KeyCollection(TwoBytesKeyMap<K, V> dictionary) => map = dictionary ?? throw new ArgumentNullException(nameof(dictionary));
        ...
        public void CopyTo(K[] dst, int dstIndex)
        {
            ArgumentNullException.ThrowIfNull(dst);
            ArgumentOutOfRangeException.ThrowIfGreaterThanOrEqual((uint)dstIndex, (uint)dst.Length);
            if( dst.Length - dstIndex < map.Count ) throw new ArgumentException("Destination array is not long enough.");
        ...
        public struct Enumerator : IEnumerator<K>{
            private TwoBytesKeyMap<K, V>.Enumerator _dictEnumerator;
            internal Enumerator(TwoBytesKeyMap<K, V> dictionary) => _dictEnumerator = dictionary.GetEnumerator();
    ...
    public sealed class ValueCollection : ICollection<V>, IReadOnlyCollection<V>{
        private readonly TwoBytesKeyMap<K, V> map;
        public ValueCollection(TwoBytesKeyMap<K, V> dictionary) => map = dictionary ?? throw new ArgumentNullException(nameof(dictionary));
        public int  Count      => map.Count;
        public bool IsReadOnly => true;

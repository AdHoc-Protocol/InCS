...
/// <summary>
/// Represents a collection of keys and values, where keys are unique nullable reference types of type <typeparamref name="K"/>.
/// This map implementation uses a custom hash table and is optimized for performance, supporting a single null key.
/// </summary>
...
public class NullableKeyMap<K, V> : IDictionary<K?, V>, IReadOnlyDictionary<K?, V>, IEquatable<NullableKeyMap<K, V>> where K : class{
...
#region Constructors
    /// <summary>
    /// Initializes a new instance of the <see cref="NullableKeyMap{K, V}"/> class that is empty, has the default initial capacity, and uses the default equality comparer for the key type.
    /// </summary>
    public NullableKeyMap() : this(0, null) { }

    /// <summary>
    /// Initializes a new instance of the <see cref="NullableKeyMap{K, V}"/> class that is empty, has the specified initial capacity, and uses the default equality comparer for the key type.
    /// </summary>
    /// <param name="capacity">The initial number of elements that the <see cref="NullableKeyMap{K, V}"/> can contain.</param>
    /// <exception cref="ArgumentOutOfRangeException">capacity is less than 0.</exception>
    public NullableKeyMap(int capacity) : this(capacity, null) { }

    /// <summary>
    /// Initializes a new instance of the <see cref="NullableKeyMap{K, V}"/> class that contains elements copied from the specified <see cref="IDictionary{K, V}"/> and uses the default equality comparer for the key type.
    /// </summary>
    /// <param name="dictionary">The <see cref="IDictionary{K, V}"/> whose elements are copied to the new <see cref="NullableKeyMap{K, V}"/>.</param>
    ...
    public NullableKeyMap(IDictionary<K?, V> dictionary) : this(dictionary, null) { }

    /// <summary>
    /// Initializes a new instance of the <see cref="NullableKeyMap{K, V}"/> class that is empty, has the default initial capacity, and uses the specified <see cref="IEqualityComparer{K}"/>.
    /// </summary>
    ...
    public NullableKeyMap(IEqualityComparer<K>? comparer) : this(0, comparer) { }

    /// <summary>
    /// Initializes a new instance of the <see cref="NullableKeyMap{K, V}"/> class that is empty, has the specified initial capacity, and uses the specified <see cref="IEqualityComparer{K}"/>.
    /// </summary>
    /// <param name="capacity">The initial number of elements that the <see cref="NullableKeyMap{K, V}"/> can contain.</param>
    /// <param name="comparer">The <see cref="IEqualityComparer{K}"/> implementation to use when comparing keys, or null to use the default <see cref="EqualityComparer{K}.Default"/> for the type of the key.</param>
    /// <exception cref="ArgumentOutOfRangeException">capacity is less than 0.</exception>
    public NullableKeyMap(int capacity, IEqualityComparer<K>? comparer)
    {
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="NullableKeyMap{K, V}"/> class that contains elements copied from the specified <see cref="IDictionary{K, V}"/> and uses the specified <see cref="IEqualityComparer{K}"/>.
    /// </summary>
    /// <param name="dictionary">The <see cref="IDictionary{K, V}"/> whose elements are copied to the new <see cref="NullableKeyMap{K, V}"/>.</param>
    /// <param name="comparer">The <see cref="IEqualityComparer{K}"/> implementation to use when comparing keys, or null to use the default <see cref="EqualityComparer{K}.Default"/> for the type of the key.</param>
    /// <exception cref="ArgumentNullException">dictionary is null.</exception>
    ...
    public int Count => _loSize + _hiSize + (_hasNullKey ?
                                                 1 :
                                                 0);
    ...
    private bool TryInsert(K? key, V value, bool overwrite)
    {
        if( key == null )
        {
            if( _hasNullKey && !overwrite ) return false;
            _nullKeyValue = value;
            if( _hasNullKey ) return true;
            _hasNullKey = true;
            _version++;
    ...
    {
        if( key               == null ) return _hasNullKey;
        ...
        {
            ref var entry = ref _entries![i];
            if( entry.hash == hashCode && _comparer.Equals(entry.key, key) ) return true;
    ...
    {
        if( _hasNullKey && EqualityComparer<V>.Default.Equals(_nullKeyValue, value) ) return true;
    ...
    public bool Remove(K? key)
    {
        ...
        {
            if( removeEntry.hash != hashCode || !_comparer.Equals(removeEntry.key, key) ) return false;
        ...
        else
        {
            ...
            if( (removeEntry = ref _entries![removeIndex = (int)link]).hash == hashCode && _comparer.Equals(removeEntry.key, key) )
            ...
            else if( _loSize <= removeIndex ) return false; // The second node was in hi-region but didn't match.
                ...
                for( var collisions = 0;; )
                {
                    ...
                    if( _loSize     <= removeIndex ) return false; // Reached end of chain.
                    if( _loSize + 1 < collisions++ ) throw new InvalidOperationException("Concurrent operations not supported.");
                }
        }
    ...
    public void CopyTo(KeyValuePair<K?, V>[] dst, int dstIndex)
    {
        ...
        if( _hasNullKey ) dst[dstIndex++] = new KeyValuePair<K?, V>(null, _nullKeyValue);
    ...
    public struct Enumerator : IEnumerator<KeyValuePair<K?, V>>{
        private readonly NullableKeyMap<K, V> _map;
        private          int                  _version;
        private          int                  _index; // -1=before null key, 0=null key, 1 to _loSize=lo-region, _entries.Length - _hiSize to _entries.Length=hi-region

        internal Enumerator(NullableKeyMap<K, V> map)
        {
            _map     = map;
            _version = map._version;
            _index = _map._hasNullKey ?
                         -2 :
                         -1;
            _current = default;
        }
        ...
        public bool MoveNext()
        {
            ...
            if( _index == int.MaxValue || _map._loSize + _map._hiSize == 0 )
            {
        ...
        public void Reset()
        {
            ...
            _index = _index = _map._hasNullKey ?
                                  -2 :
                                  -1;
            _current = default;
        }
    ...
    public void TrimExcess()
    {
        ...
        if( _entries != null && GetPowerOfTwo(count) < _entries.Length ) Resize(GetPowerOfTwo(count));
    }

#region Equality
    public bool Equals(NullableKeyMap<K, V>? other)
    {
        if( other       == null              || Count != other.Count || !_comparer.Equals(other._comparer) ) return false;
        if( _hasNullKey != other._hasNullKey || _hasNullKey && !EqualityComparer<V>.Default.Equals(_nullKeyValue, other._nullKeyValue) ) return false;

        // Count is the same, null key state is the same. Now check all other keys.
        foreach( var pair in this )
        {
            if( pair.Key is null ) continue; // Already checked null case
            if( !other.TryGetValue(pair.Key, out var otherValue) || !EqualityComparer<V>.Default.Equals(pair.Value, otherValue) ) return false;
        }

        return true;
    }

    public override bool Equals(object? obj) => Equals(obj as NullableKeyMap<K, V>);
    ...
    public override int GetHashCode()
    {
        ...
        if( _hasNullKey )
            // Hash for the null key pair. Key hash is 0.
            hashCode ^= _nullKeyValue?.GetHashCode() ?? 0;
...
#region Helper Collections (Unchanged, work as-is)
    ...
    public sealed class KeysCollection : ICollection<K?>, IReadOnlyCollection<K?>{
        private readonly NullableKeyMap<K, V> map;
        public KeysCollection(NullableKeyMap<K, V> map) => this.map = map;
        ...
        public struct Enumerator : IEnumerator<K?>{
            private NullableKeyMap<K, V>.Enumerator _dictEnumerator;
            internal Enumerator(NullableKeyMap<K, V> dictionary) => _dictEnumerator = dictionary.GetEnumerator();
    ...
    public sealed class ValuesCollection : ICollection<V>, IReadOnlyCollection<V>{
        private readonly NullableKeyMap<K, V> map;
        public ValuesCollection(NullableKeyMap<K, V> map) => this.map = map;
        public int  Count      => map.Count;
        public bool IsReadOnly => true;

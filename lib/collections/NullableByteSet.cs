...
public struct NullableByteSet<K> : ISet<K?>, IReadOnlySet<K?> where K : unmanaged{
    ...
    {
        if( other._hasNullKey && !_hasNullKey ) return false; // If other has null but we don't
    ...
    {
        _bits &= ~other._bits;
        if( other._hasNullKey ) _hasNullKey = false; // If other had null, we remove null from ourselves
        _version++;
        _count = -2; // Mark _count for recalculation
    }
    ...
    internal int Next1(int key)
    {
        ...
        if( word != 0 ) return key + BitOperations.TrailingZeroCount(word); // Found in current word

        for( var i = index + 1; i < 4; i++ )
        {
            word = _bits.GetElement(i);
            if( word != 0 ) return (i << 6) + BitOperations.TrailingZeroCount(word); // Found in a subsequent word
        }
    ...
    public bool Remove(K? key)
    {
        ...
        // key is null
        if( !_hasNullKey ) return false; // Doesn't have null
        _hasNullKey = false;
    ...
    public void CopyTo(K?[] dst, int dstIndex)
    {
        ...
        var byteKeyCount = Count - (_hasNullKey ?
                                        1 :
                                        0);
        ...
        for( var i = 0; i < 4; i++ )
        {
            ...
            for( var j = 0; j < 64; j++ )
            {
                if( (segment & (1UL << j)) != 0 ) // Check the j-th bit in the current segment (multiple statements, so braces needed)
                {
                    ...
                    copiedByteKeys++;
                    if( copiedByteKeys == byteKeyCount ) return; // Single statement, no braces needed here
                }
            }
        }
    }
    ...
    public void Clear()
    {
        if( Count == 0 ) return; // Only clear if not already empty. Count recalculates, so this is safe.

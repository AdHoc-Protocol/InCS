...
public interface BitList{
    ...
    public abstract class R : ICloneable, IReadOnlyList<bool>, IEquatable<R>{
        ...
        protected static ulong Mask(int bits) => bits >= BITS ?
                                                     ~0UL :
                                                     (1UL << bits) - 1;
        ...
        {
            if( bit < 0 || bit >= count ) return false;
        ...
        public int Get(int bit, int FALSE, int TRUE) => Get(bit) ?
                                                            TRUE :
                                                            FALSE;
        ...
        {
            if( toBit <= fromBit || fromBit < 0 || Count <= fromBit ) return 0;
            ...
            if( remainingBits == 0 ) return copiedBits;
        ...
        {
            if( count == 0 || bit++ < -1 || count <= bit ) return -1;
        ...
        public int Next0(int bit)
        {
            ...
            if( ++bit < trailingOnesCount )
                return trailingOnesCount == count ?
                           -1 :
                           trailingOnesCount;

            if( count <= bit ) return -1;

            var last1 = Last1;

            if( bit == last1 )
                return last1 + 1 < count ?
                           bit + 1 :
                           -1;
        ...
        public int Prev1(int bit)
        {
            ...
            for( var value = values[index] & Mask((bitOffset & MASK) + 1);; value = values[--index] )
                if( value == 0 )
                {
                    if( index == 0 ) return trailingOnesCount - 1;
                }
                else
        ...
        /// </value>
        public int Last1 => Used() == 0 ?
                                trailingOnesCount                            - 1 :
                                trailingOnesCount + (used - 1 << LEN) + BITS - 1 - BitOperations.LeadingZeroCount(values[used - 1]);
        ...
        {
            if( bit < 0 || count == 0 ) return 0;
            if( count  <= bit ) bit = count - 1;
        ...
        {
            if( cardinality <= 0 || Cardinality < cardinality ) return -1;

            if( cardinality <= trailingOnesCount ) return cardinality - 1;
        ...
        {
            if( other == null || Count != other.Count || trailingOnesCount != other.trailingOnesCount ) return false;
            for( var i = Used(); -1 < --i; )
        ...
        public StringBuilder ToJson(StringBuilder sb)
        {
            ...
            if( 0 < count )
            {
                for( var i = 0; i < trailingOnesCount; i++ ) sb.Append(1).Append(',');
                var last1 = Last1;
                ...
                for( var i = last1 + 1; i < count; i++ ) sb.Append(0).Append(',');
        ...
        /// is '1'.</returns>
        public int NumberOfLeading0 => count == 0 ?
                                           0 :
                                           count - 1 - Last1;
        ...
        public int NumberOfTrailing0
        {
            get
            {
                if( count == 0 || 0 < trailingOnesCount ) return 0;
                var i = Next1(-1);
                return i == -1 ?
                           count :
                           i;
        ...
        {
            if( bit + BITS <= trailingOnesCount ) return ulong.MaxValue;
    ...
    public class RW : R, IList<bool>{
        ...
        public RW(int bits)
        {
            ...
            if( bits > 0 ) values = new ulong[Len4Bits(bits)];
        }
        ...
        public RW And(R and)
        {
            ...
            if( and.trailingOnesCount < trailingOnesCount ) { Set0(and.trailingOnesCount); }
        ...
        public RW Xor(R xor)
        {
            ...
            var newValuesLen = Len4Bits(newToc <= maxLast1 ?
                                            maxLast1 - newToc + 1 :
                                            0);
            var i = 0;
            ...
            if( newToc < trailingOnesCount )
            {
                ...
                values = 0 < Used() ?
                             ShiftLeft(values, 0, last1 - trailingOnesCount + 1, shiftBits, false) :
                             values.Length < sb ?
                                 new ulong[sb] :
                                 values;
        ...
        public bool Intersects(R other)
        {
            ...
            var thisWordEndIndex = thisRelBitEnd < 0 ?
                                       -1 :
                                       Math.Min(thisUsed - 1, thisRelBitEnd >> LEN);
            var otherWordEndIndex = otherRelBitEnd < 0 ?
                                        -1 :
            ...
            {
                var thisWord = wordIndex >= thisWordStartIndex && wordIndex <= thisWordEndIndex ?
                                   values[wordIndex] :
                                   0UL;

                var otherWord = wordIndex >= otherWordStartIndex && wordIndex <= otherWordEndIndex ?
        ...
        public RW Flip(int fromBit, int toBit)
        {
            ...
            if( fromBit == trailingOnesCount )
            {
                ...
                values = ShiftRight(values, values.Length < len ?
                                                new ulong[len] :
                                                values, 0, last1 - trailingOnesCount + 1, shiftBits, true);
            ...
            if( last1 < toBit )
            {
                ...
                if( values.Length < u )
                    if( used == 0 ) values = new ulong[u];
                    else Array.Resize(ref values, u);
        ...
        public RW Set1(int bit)
        {
            ...
            if( values.Length < index + 1 ) Array.Resize(ref values, Math.Max(values.Length * 3 / 2, index + 1));
        ...
        public RW Set1(int fromBit, int toBit)
        {
            if( fromBit < 0 || toBit <= fromBit ) return this;
            if( count < toBit ) count = toBit;
            if( toBit <= trailingOnesCount ) return this;
            ...
            if( fromBit <= trailingOnesCount )
            {
                ...
                if( Used() > 0 ) values = ShiftRight(values, values, 0, last1 - trailingOnesCount + 1, toBit - trailingOnesCount, true);
            ...
            if( values.Length < max + 1 ) Array.Resize(ref values, Math.Max(values.Length * 3 / 2, max + 1));
        ...
        public RW Set0(int bit)
        {
            ...
            if( bit < trailingOnesCount )
            {
                ...
                if( bitsInValues       > 0 ) values    = ShiftLeft(values, 0, bitsInValues, shift, trailingOnesCount == 1);
                else if( values.Length < used ) values = new ulong[used];

                if( shift > 1 ) Fill(1, values, 1, shift);
        ...
        public RW Set0(int fromBit, int toBit)
        {
            ...
            var bitsInValues = last1InValue < 0 ?
                                   0 :
                                   last1InValue + 1;
            ...
            if( toBit <= trailingOnesCount )
            {
                ...
                if( bitsInValues       > 0 ) values    = ShiftLeft(values, 0, bitsInValues, toBit - fromBit + shift, true);
                else if( values.Length < used ) values = new ulong[Math.Max(values.Length         + (values.Length >> 1), used)];

                if( shift > 0 ) Fill(1, values, toBit - fromBit, toBit - fromBit + shift);
            }
            else if( fromBit < trailingOnesCount )
            {
                ...
                if( bitsInValues       > 0 ) values    = ShiftLeft(values, 0, bitsInValues, shift, true);
                else if( values.Length < used ) values = new ulong[Math.Max(values.Length + (values.Length >> 1), used)];
        ...
        public RW Add0(int bit)
        {
            ...
            if( bit < trailingOnesCount )
            {
                ...
                if( last1InValues      > 0 ) values    = ShiftLeft(values, 0, last1InValues + 1, shiftBits, true);
                else if( values.Length < used ) values = new ulong[used];
        ...
        public RW Add1(int bit)
        {
            ...
            if( Used() == 0 )
            {
                if( values.Length < index + 1 ) values = new ulong[index + 1];
            }
        ...
        public RW Capacity_(int bits)
        {
            ...
            else if( bits < count )
            {
                ...
                if( len < values.Length ) Array.Resize(ref values, len);
        ...
        private static void Fill(int src, ulong[] dst, int loBit, int hiBit)
        {
            ...
            switch( src )
            {
                case 0:
                    ...
                    for( var i = loIndex + 1; i < hiIndex; i++ ) dst[i] = 0UL;
        ...
        {
            if( hiBit <= loBit || shiftBits < 1 ) return src;

            if( src != dst && loBit > 0 ) Array.Copy(src, 0, dst, 0, Len4Bits(loBit));
            if( shiftBits < hiBit - loBit ) BitCopy(src, loBit + shiftBits, dst, loBit, hiBit - loBit - shiftBits);

            if( clear ) Fill(0, dst, hiBit - shiftBits, hiBit);
            return dst;
        }
        ...
        internal static ulong[] ShiftLeft(ulong[] src, int loBit, int hiBit, int shiftBits, bool clear)
        {
            ...
            if( src.Length < max )
            {
                ...
                if( loBit >> LEN > 0 ) Array.Copy(src, 0, dst, 0, loBit >> LEN);
            }

            BitCopy(src, loBit, dst, loBit + shiftBits, hiBit - loBit);

            if( clear ) Fill(0, dst, loBit, loBit + shiftBits);
        ...
        public int IndexOf(bool item) => item ?
                                             Next1(-1) :
                                             Next0(-1);
    ...
    public static int Resize<T>(T[] src, T[] dst, int index, int count, int resize)
    {
        ...
        if( resize < 0 )
        {
            ...
            if( count <= index + -resize ) return index;
            Array.Copy(src, index + -resize, dst, index, count - (index + -resize));
            return count + resize;
        }

        if( count < 1 ) return Math.Max(index, count) + resize;
        ...
        if( index < count )
            ...
            {
                if( src != dst && 0 < index ) Array.Copy(src, 0, dst, 0, index);

...
/// <summary>
/// Represents a collection of keys and values, where keys are unique nullable primitive values of type <typeparamref name="K"/>.
/// This map implementation uses a hash table and is optimized for nullable primitive keys.
/// It supports a single null key.
/// </summary>
...
public class NullablePrimitiveKeyMap<K, V> : IDictionary<K?, V>, IReadOnlyDictionary<K?, V> where K : unmanaged{
    ...
    public NullablePrimitiveKeyMap() : this(0) { }

    public NullablePrimitiveKeyMap(int capacity)
    {
        ArgumentOutOfRangeException.ThrowIfNegative(capacity);
        if( capacity > 0 ) Initialize(capacity);
    }

    public NullablePrimitiveKeyMap(IEnumerable<KeyValuePair<K?, V>> collection) : this(collection is ICollection<KeyValuePair<K?, V>> coll ?
                                                                                           coll.Count :
                                                                                           0)
    {
    ...
    public NullablePrimitiveKeyMap(IDictionary<K?, V> dictionary) : this(dictionary?.Count ?? 0)
    {
    ...
    private bool TryInsert(K? key, V value, bool overwrite)
    {
        ...
        else                                                           //Collision detected (bucket is not empty).
        {
            ...
            for( uint i = bucket - 1, collisions = 0;; )
            {
                ...
                if( EqualityComparer<K>.Default.Equals(entry.key, kv) )
                {
    ...
    {
        if( !key.HasValue ) return _hasNullKey;
        var kv = key.Value;
        ...
        {
            if( EqualityComparer<K>.Default.Equals(_entries![i].key, kv) ) return true;
    ...
    public bool Remove(K? key)
    {
        ...
        {
            if( !EqualityComparer<K>.Default.Equals(removeEntry.key, kv) ) return false;

            // An item from hi-region is being removed. The last hi-region item will be moved to its place.
        ...
        if( EqualityComparer<K>.Default.Equals(removeEntry.key, kv) ) // Head of chain matches
        ...
        else
        {
            ...
            if( EqualityComparer<K>.Default.Equals((removeEntry = ref _entries![removeIndex = (int)link]).key, kv) )
                ...
                for( var collisions = 0;; )
                {
                    ...
                    if( EqualityComparer<K>.Default.Equals((removeEntry = ref _entries![removeIndex = (int)(link = ref _links![last = removeIndex])]).key, kv) )
                    {
    ...
    void ICollection<KeyValuePair<K?, V>>.CopyTo(KeyValuePair<K?, V>[] array, int index)
    {
        ...
        if( array.Length - index < Count ) throw new ArgumentException("Destination array is not long enough.");

        if( _hasNullKey ) array[index++] = new KeyValuePair<K?, V>(null, _nullKeyValue);
    ...
    public struct Enumerator : IEnumerator<KeyValuePair<K?, V>>{
        private readonly NullablePrimitiveKeyMap<K, V> _map;
        private          int                           _version;
        private          int                           _index; // -2=before null key, -1=null key or initial state if no null key, 0 to _loSize=lo-region, _entries.Length - _hiSize to _entries.Length=hi-region
        private          KeyValuePair<K?, V>           _current;

        internal Enumerator(NullablePrimitiveKeyMap<K, V> map)
        {
            _map     = map;
...
#region Helper Collections
    ...
    public sealed class KeysCollection : ICollection<K?>, IReadOnlyCollection<K?>{
        private readonly NullablePrimitiveKeyMap<K, V> map;
        public KeysCollection(NullablePrimitiveKeyMap<K, V> map) => this.map = map;
        ...
        public struct Enumerator : IEnumerator<K?>{
            private NullablePrimitiveKeyMap<K, V>.Enumerator _dictEnumerator;
            internal Enumerator(NullablePrimitiveKeyMap<K, V> dictionary) => _dictEnumerator = dictionary.GetEnumerator();
    ...
    public sealed class ValuesCollection : ICollection<V>, IReadOnlyCollection<V>{
        private readonly NullablePrimitiveKeyMap<K, V> map;
        public ValuesCollection(NullablePrimitiveKeyMap<K, V> map) => this.map = map;
        public int  Count      => map.Count;
        public bool IsReadOnly => true;

...
public class ByteMap<K, V> : IDictionary<K, V>, IReadOnlyDictionary<K, V>, IEquatable<ByteMap<K, V>> where K : unmanaged{
    ...
    public V this[K key]
    {
        get
        {
            ...
            return keys.Get(_key) && nullsVal.Get(_key) ?
                       values[IsFlat ?
                                  _key :
    ...
    private class ValueCollection : ICollection<V>{
        ...
        private struct ValueEnumerator : IEnumerator<V>{
            ...
            public V Current => _version != _map.keys._version ?
                                    throw new InvalidOperationException("Collection was modified during enumeration.") :
                                    _key == ByteSet<K>.INVALID ? // Check if enumeration has not started or has finished
                                        throw new InvalidOperationException("Enumeration has either not started or has finished.") :
                                        currentValue;

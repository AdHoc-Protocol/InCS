...
/// </summary>
public class NullableTwoBytesKeyMap<K, V> : IDictionary<K?, V>, IReadOnlyDictionary<K?, V> where K : unmanaged{
    ...
    private bool TryInsert(K? key, V value, bool overwrite)
    {
        ...
        while( true )
        {
            ...
            else if( _entries!.Length <= _loSize + _hiSize )
            {
                ...
                if( newSize > flatStrategyThreshold && _entries.Length < flatStrategyThreshold ) newSize = flatStrategyThreshold;
                Resize(newSize);
    ...
    public bool ContainsValue(V value)
    {
        ...
        if( IsFlatStrategy )
        {
            ...
            {
                var bits = _nulls![i];
                if( bits == 0 ) continue; // Skip empty 64-bit segments

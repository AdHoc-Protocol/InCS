...
public class NullableSet<K> : ISet<K?>, IReadOnlyCollection<K?>, IEquatable<NullableSet<K>> where K : class{
...
#region Constructors
    ...
    public bool Add(K? item)
    {
        ...
        if( _entries!.Length <= _loSize + _hiSize ) Resize(_entries.Length * 2);
        ...
        if( index != -1 )
            for( int i = index, collisions = 0;; )
            {
                ...
                if( entry.hash == hashCode && _comparer.Equals(entry.key, item) ) return false; // Already exists
        ...
        int dstIndex;
        if( index == -1 ) dstIndex = _entries.Length - 1 - _hiSize++; // Bucket is empty: place new entry in hi-region
    ...
    {
        if( item              == null ) return _hasNullKey;
        ...
        {
            ref var entry = ref _entries![i];
            if( entry.hash == hashCode && _comparer.Equals(entry.key, item) ) return true;
    ...
    public void CopyTo(K?[] dst, int dstIndex)
    {
        ...
        for( var i = 0; i                         < _loSize; i++ ) dst[dstIndex++] = _entries[i].key;
        if( _hiSize                               == 0 ) return;
        for( var i = _entries.Length - _hiSize; i < _entries.Length; i++ ) dst[dstIndex++] = _entries[i].key;
    }
...
#region ISet<T> Implementation
    ...
    {
        if( other is ICollection<K?> coll ) return (coll.Count, other);
        if( other is IReadOnlyCollection<K?> roColl ) return (roColl.Count, other);
...
#region Equality
    public bool Equals(NullableSet<K>? other)
    {
        ...
        if( !_comparer.Equals(other._comparer) ) return false;
    ...
    public override int GetHashCode()
    {
        ...
        if( _hasNullKey ) hashCode ^= 0; // Hash for the null item

        if( _entries is null ) return hashCode;

        // Iterate lo-region
        for( var i = 0; i < _loSize; i++ ) hashCode ^= _entries[i].hash;
        if( _hiSize       == 0 ) return hashCode;

        // Iterate hi-region
        for( var i = _entries.Length - _hiSize; i < _entries.Length; i++ ) hashCode ^= _entries[i].hash;

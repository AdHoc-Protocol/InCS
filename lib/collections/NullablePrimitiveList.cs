...
public interface NullablePrimitiveList<T> where T : struct{
    /// <summary>
    ...
    abstract class R : ICloneable, IReadOnlyList<T?>, IEquatable<R>{
        ...
        public int Cardinality => isFlatStrategy ?
                                      nulls.Cardinality :
                                      cardinality;
        ...
        protected void SwitchToFlatStrategy(int capacity)
        {
            ...
            {
                if( values.Length == capacity ) values = new T [16]; // Ensure minimum capacity if needed.
                isFlatStrategy = true;
        ...
        public virtual T? this[int index]
        {
            get
            {
                ...
                if( index >= Count ) throw new ArgumentOutOfRangeException(nameof(index), "Index is out of bounds.");
        ...
        {
            if( !HasValue(index) ) return ref Unsafe.NullRef<T>();
            if( isFlatStrategy ) return ref values[index];
        ...
        /// otherwise, <c>false</c>.
        /// </returns>
        public bool Contains(T? value) => value                 == null ?
                                              NextNullIndex(-1) != -1 :
        ...
        {
            if( value == null ) return PrevNullIndex(Count);
        ...
        public override int GetHashCode()
        {
            ...
            for( int i = 0, s = nulls.Count, ii = 0; i < s; i++ )
                hash = HashCode.Combine(hash, nulls![i] ?
                                                  EqualityComparer<T>.Default.GetHashCode(values[isFlatStrategy ?
                                                                                                     i :
                                                                                                     ii++]) :
                                                  300463);
        ...
        public bool Equals(R? other)
        {
            ...
            if( other == null || Count != other.Count || !nulls.Equals(other.nulls) ) return false;
            if( isFlatStrategy )
            {
        ...
        protected static void Set(R dst, int index, T? value)
        {
            ...
            if( value == null )
            {
                ...
                {
                    if( dst.values.Length <= dst.nulls.Count ) Array.Resize(ref dst.values, Math.Max(index + 1, dst.values.Length * 3 / 2 + 1));
                }
            ...
            {
                if( dst.values.Length <= index ) Array.Resize(ref dst.values, Math.Max(index + 1, dst.values.Length * 3 / 2));

                dst.values[index] = value.Value;
                dst.nulls.Set1(index);
            }
            else if( dst.nulls.Get(index) ) dst.values[dst.nulls.Rank(index) - 1] = value.Value;
            else
            {
                ...
                else
                {
                    ...
                    dst.cardinality = BitList.Resize(dst.values,
                                                     dst.values.Length <= dst.cardinality ?
                                                         dst.values = new T[dst.cardinality * 3 / 2 + 2] :
                                                         dst.values, rank, dst.cardinality, 1);
    ...
    /// </summary>
      class RW : R, IList<T?>{
        ...
        public RW Remove(int index)
        {
            ...
            if( isFlatStrategy ) BitList.Resize(values, values, index, nulls.count, -1);
            else if( nulls.Get(index) ) cardinality = BitList.Resize(values, values, nulls.Rank(index) - 1, cardinality, -1);
        ...
        public RW Set(int index, params T?[]? values)
        {
            ...
            for( var i = 0; i < values.Length; i++ ) Set(this, index + i, values[i]);
            return this;
        }
        ...
        public RW Set(int index, T[] src, int srcIndex, int len)
        {
            ...
            if( srcIndex + len > src.Length ) throw new ArgumentOutOfRangeException(nameof(len), "Source range exceeds array bounds.");

            for( var i = len; -1 < --i; ) Set(this, index + i, src[srcIndex + i]);
            return this;
        }
        ...
        public RW Add(int index, T? value)
        {
            ...
            if( isFlatStrategy )
            {
                BitList.Resize(values, values.Length <= nulls.Count ?
                                           new T[Math.Max(16, nulls.Count * 3 / 2)] :
                                           values, index, nulls.Count, 1);
                if( value == null ) nulls.Insert(index, false);
                else
                {
            ...
            else
            {
                ...
                if( index <= nulls.Last1 ) i--;
                cardinality = BitList.Resize(values, values.Length <= cardinality ?
                                                         values = new T [Math.Max(16, cardinality * 3 / 2)] :
                                                         values, i, cardinality, 1);
                nulls.Insert(index, true);
        ...
        public RW Capacity_(int capacity)
        {
            ...
            if( isFlatStrategy )
            {
                ...
                else if( values.Length != capacity ) Array.Resize(ref values, capacity);
            }
            else
            {
                if( shrink ) cardinality = nulls.Cardinality;
                if( flatStrategyThreshold <= cardinality ) SwitchToFlatStrategy(capacity);
                else if( values.Length    != capacity ) Array.Resize(ref values, capacity);
            }
        ...
        public RW Fit()
        {
            ...
            if( !isFlatStrategy || flatStrategyThreshold < nulls.Cardinality ) return this;
        ...
        public RW Swap(int index1, int index2)
        {
            ...
            if( val1 == null && val2 == null || val1 != null && val2 != null && EqualityComparer<T>.Default.Equals(val1!.Value, val2!.Value) ) return this;
        ...
        public override T? this[int index] { get => base[index]; set => Set(this, index, value); }
        ...
        public void CopyTo(T?[] dst, int dstIndex)
        {
            ...
            for( var i = 0; i < Count; i++ ) dst[dstIndex + i] = this[i];
        }

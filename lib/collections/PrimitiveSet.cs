...
public class PrimitiveSet<K> : ISet<K>, IReadOnlyCollection<K> where K : unmanaged{
    ...
    public PrimitiveSet(IEnumerable<K> collection) : this(collection is ICollection<K> coll ?
                                                              coll.Count :
                                                              0)
    {
    ...
    public bool Add(K key)
    {
        ...
        else
        {
            ...
            {
                if( EqualityComparer<K>.Default.Equals(_keys[i], key) ) return false;

                if( _loSize <= i ) break; // Reached a hi-region entry, end of this chain.
                i = (int)_links![i];
    ...
    public bool Contains(K key)
    {
        ...
        {
            if( EqualityComparer<K>.Default.Equals(_keys![i], key) ) return true;

            if( _loSize <= i ) return false; // Reached end of chain (a hi-region entry)
            i = (int)_links![i];
    ...
    private void Resize(int newSize)
    {
        ...
        for( var i = 0; i                           < oldLoSize; i++ ) Copy(in oldKeys![i]);
        for( var i = oldKeys!.Length - oldHiSize; i < oldKeys.Length; i++ ) Copy(in oldKeys[i]);
    }
    ...
    public bool Remove(K key)
    {
        ...
        {
            if( !EqualityComparer<K>.Default.Equals(removeKey, key) ) return false;
    ...
    public struct Enumerator : IEnumerator<K>{
        ...
        public K Current => _version != _set._version ?
                                ...
                                _index == -1 || _index == _set._keys!.Length ?
...
#region ISet Explicit/Remaining Implementation
    ...
    public void ExceptWith(IEnumerable<K> other)
    {
        ...
        if( Count == 0 ) return;
        if( other is PrimitiveSet<K> otherSet && otherSet.Count == 0 ) return;

...
public interface NullableObjectList<T> where T : class{
    /// <summary>
    ...
    public abstract class R : ICloneable, IReadOnlyList<T?>, IEquatable<R>{
        ...
        protected void SwitchToFlatStrategy(int capacity)
        {
            if( Count == 0 )
            {
                if( values.Length == 0 ) values = new T?[16];
                isFlatStrategy = true;
                return;
            }
        ...
        public T?[] ToArray(int index, int len, T?[]? dst)
        {
            ...
            if( dst == null || dst.Length < len ) dst = new T?[len];
        ...
        public int Count => isFlatStrategy ?
                                size_card :   // In flat mode, size_card is the total count.
        ...
        public int Cardinality => isFlatStrategy ?
                                      values.Count(v => v != null) : // Need to count in flat mode.
        ...
        public bool HasValue(int index) => 0 <= index && index < Count && ( // Index out of bounds.
                                                                              isFlatStrategy ?
                                                                                  values[index] != null : // Flat: Check value directly.
        ...
        public int IndexOf(T? value)
        {
            ...
            if( value == null ) return NextNullIndex(-1); // Search for the first null index.
        ...
        public override int GetHashCode()
        {
            ...
            if( isFlatStrategy )
                ...
                for( int i = 0, s = nulls.Count; i < s; i++ )
                    hash = HashCode.Combine(hash, equal_hash_V.GetHashCode(nulls![i] ?
                                                                               values[i] :
                                                                               null));
        ...
        protected static void Set(R dst, int index, T? value)
        {
            ...
            if( dst.isFlatStrategy )
            {
                ...
                else if( dst.size_card <= index ) dst.size_card = index + 1;
            ...
            else
            {
                ...
                dst.size_card = BitList.Resize(dst.values,
                                               dst.values.Length == dst.size_card ?
                                                   dst.values = new T[Math.Max(16, dst.size_card * 3 / 2)] :
                                                   dst.values,
    ...
    public class RW : R, IList<T?>{
        ...
        public void FlatStrategyThreshold(int threshold)
        {
            ...
            else if( (flatStrategyThreshold = threshold) <= size_card ) SwitchToFlatStrategy();
        }
        ...
        {
            ArgumentOutOfRangeException.ThrowIfNegative(index);
            if( Count < 1 || Count <= index ) return this;
            if( isFlatStrategy )
                size_card = BitList.Resize(values, values, index, size_card, -1);
            else
            {
                if( nulls.Get(index) ) size_card = BitList.Resize(values, values, nulls.Rank(index) - 1, size_card, -1);
        ...
        {
            ArgumentOutOfRangeException.ThrowIfNegative(index);
            for( var i = src.Length; -1 < --i; ) Set(this, index + i, src[i]);
            return this;
        }
        ...
        public RW Set(int index, T?[] src, int src_index, int len)
        {
            ...
            if( src_index + len > src.Length ) throw new ArgumentOutOfRangeException(nameof(len), "Source array range is out of bounds.");

            for( var i = len - 1; -1 < i; --i ) Set1(index + i, src[src_index + i]);
            return this;
        }
        ...
        public RW Add1(int index, T? value)
        {
            ...
            if( isFlatStrategy )
            {
                size_card = BitList.Resize(values, values.Length <= size_card ?
                                                       new T[Math.Max(16, size_card * 3 / 2)] :
            ...
            else
            {
                ...
                size_card = BitList.Resize(values, values.Length <= size_card ?
                                                       values = new T [Math.Max(16, size_card * 3 / 2)] :
        ...
        /// <returns>This <see cref="RW"/> instance, allowing for method chaining.</returns>
        public RW Add(params T?[]? items) => items == null ?
                                                 this :
                                                 Set(Count, items);
        ...
        public void Capacity_(int capacity)
        {
            ...
            if( values.Length != capacity ) Array.Resize(ref values, capacity);
        }
        ...
        public RW Count_(int count)
        {
            ...
            {
                if( values.Length != count ) Array.Resize(ref values, count);
        ...
        public RW Fit()
        {
            ...
            if( !isFlatStrategy )
            {
                if( values.Length != size_card ) Array.Resize(ref values, size_card);
                return this;
            }

            if( flatStrategyThreshold < Cardinality ) return this;

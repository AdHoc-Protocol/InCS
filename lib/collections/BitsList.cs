...
public interface BitsList<T> where T : struct{
    ...
    public abstract class R : ICloneable, IReadOnlyList<T>, IEquatable<R>{
        ...
        protected void Capacity(int items)
        {
            if( 0 < items )
            {
                ...
                if( values.Length != newLength ) Array.Resize(ref values, (int)newLength);
            }
            else
            {
                var newLength = LengthForBits((uint)(-items * bitsPerItem));
        ...
        {
            if( 0 < count ) Array.Fill(values, 0UL, 0, BitList.R.Len4Bits(count * bitsPerItem));
            count = 0;
        }
        ...
        {
            if( count == 0 ) return 149989999;
            var hash                          = HashCode.Combine(149989999, Count);
            var i                             = Index(Count * bitsPerItem);
            if( i      < values.Length ) hash = HashCode.Combine(hash, values[i] & Mask(Bit(Count * bitsPerItem)));
            while( --i >= 0 ) hash            = HashCode.Combine(hash, values[i]); // Hash the rest of the ulong array
            return hash;
        }
        ...
        public bool Equals(R? other)
        {
            ...
            if( values.Length > lastLongIndex && other.values.Length > lastLongIndex && (values[lastLongIndex] & lastMask) != (other.values[lastLongIndex] & lastMask) ) return false;
        ...
        public virtual T this[int item]
        {
            get
            {
                if( item < 0 || count <= item ) throw new IndexOutOfRangeException($"Index {item} is out of range [0, {count})");
                var bitPosition = item * bitsPerItem;
                var index       = Index(bitPosition);
                var bitOffset   = Bit(bitPosition);
                return BITS < bitOffset + bitsPerItem ?
                           GetValue(values[index], values[index + 1], bitOffset, bitsPerItem, mask) :
                           GetValue(values[index], bitOffset,         mask);
            }
        ...
        {
            if( item < 0 ) throw new IndexOutOfRangeException("Index cannot be negative.");
            if( dst.count < item )
            {
        ...
        {
            for( var i = 0; i < src.Length; i++ ) Set1(dst, from + i, src[i]);
        }
        ...
        {
            for( var i = 0; i < src.Length; i++ ) Set1(dst, from + i, FromByte(src[i]));
        }
        ...
        protected static void Set1(R dst, int item, T src)
        {
            ...
            if( dst.Capacity() <= item ) dst.Capacity((int)Math.Max(dst.Capacity() * 3 / 2, LengthForBits(totalBits + dst.bitsPerItem) * BITS / dst.bitsPerItem)); // Ensure capacity in items
        ...
        {
            if( item < 0 || item >= dst.count ) throw new IndexOutOfRangeException($"Index {item} is out of range [0, {dst.count})");

            if( item == dst.count - 1 )
            {
                if( ToByte(dst.default_value) == 0 ) Set_(dst, (uint)item, default);
                dst.count--;
                return;
            }
        ...
        public object Clone()
        {
            try
            {
                ...
                if( dst.Capacity() > 0 ) dst.values = (ulong[])values.Clone();
                return dst;
            }
            catch( Exception e )
            {
        ...
        {
            if( dst == null ) dst = new StringBuilder(Count * 4); // Initialize StringBuilder if null, estimate capacity
            else dst.EnsureCapacity(dst.Length + Count      * 4); // Ensure capacity

            var currentULong = values.Length > 0 ?
                                   values[0] :
                                   0; // Avoid index out of range if values is empty
            ...
            for( int itemIndex = 0, bitPosition = 0; itemIndex < Count; itemIndex++, bitPosition += bitsPerItem )
            {
                ...
                var itemValue = BITS < bitOffset + bitsPerItem ?
                                    GetValue(currentULong, nextULongIndex < values.Length ?
                                                               values[nextULongIndex] :
                                                               0, bitOffset, bitsPerItem, mask) // Handle boundary and array bounds
                                    :
                                    GetValue(currentULong, bitOffset, mask);

                dst.Append(itemValue).Append('\t');
                if( (itemIndex + 1) % 10 == 0 ) dst.Append('\t').Append((itemIndex + 1) / 10 * 10).Append('\n');
                if( BITS < bitOffset + bitsPerItem && nextULongIndex < values.Length ) currentULong = values[nextULongIndex]; // Update currentULong if needed and within bounds
            }
        ...
        {
            for( var i = dst.count; -1 < (i = dst.LastIndexOf(i, value)); ) RemoveAt(dst, i);
        }
        ...
        {
            if( count == 0 ) return [];                  // Return empty array instead of null
            dst ??= new T[count];                        // Create new array if dst is null or too small
            if( dst.Length < count ) dst = new T[count]; // Ensure dst is large enough

            for( var i = 0; i < count; i++ ) dst[i] = Get(i);
            return dst;
        }
    ...
    public class RW : R, IList<T>{
        ...
        {
            if( count           < 0 ) Clear();
            else if( this.count < count ) Set1(this, count - 1, default_value);
        ...
        /// <inheritdoc/>
        public new IEnumerator<T> GetEnumerator() { return new Enumerator(this); }

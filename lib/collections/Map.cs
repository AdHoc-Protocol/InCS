...
public class Map<K, V> : IDictionary<K, V>, IReadOnlyDictionary<K, V>, IEquatable<Map<K, V>> where K : class{
...
#region Constructors
    ...
    private bool TryInsert(K key, V value, bool overwrite)
    {
        ...
        int dstIndex;
        if( bucket == 0 ) dstIndex = _entries.Length - 1 - _hiSize++; // Bucket is empty: Place new entry in the Hi-Region (non-colliding entries).dstIndex = _entries.Length - 1 - _hiSize++;
    ...
    public struct Enumerator : IEnumerator<KeyValuePair<K, V>>{
        ...
        public KeyValuePair<K, V> Current => _version != _map._version ?
                                                 ...
                                                 _index == int.MaxValue - 1 || _index == -1 ?

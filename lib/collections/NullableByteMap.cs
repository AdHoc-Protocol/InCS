...
public class NullableByteMap<K, V> : IDictionary<K?, V?>, IReadOnlyDictionary<K?, V?>, IEquatable<NullableByteMap<K, V>> where K : unmanaged{
    ...
    {
        if( Unsafe.SizeOf<K>() != 1 ) throw new ArgumentException("ByteSet only supports one byte type as key types.");
    ...
    private static int GetPowerOfTwo(int capacity) => capacity <= 0 ?
                                                          16 :
    ...
    public bool Equals(NullableByteMap<K, V>? other)
    {
        ...
        for( int t = -1, i = 0; (t = nullsVal.Next1(t)) != -1; i++ )
            if( !equal_hash_V.Equals(IsFlat ?
                                         values[t] :
                                         values[i], other.IsFlat ?
                                                        other.values[t] :
                                                        other.values[i]) )
                return false;
        return true;
    ...
    public V? this[K? key]
    {
        get
        {
            ...
            if( !keys.Get(_key) ) return default;

            return nullsVal.Get(_key) ?
                       values[IsFlat ?
                                  _key :
                                  nullsVal.Rank(_key) - 1] :
                       default;
        }
        set => Add_(key, value);
    }
    ...
    public bool Add_(K? key, V? value)
    {
        ...
        {
            if( nullsVal.Set0(_key) && !IsFlat ) BitList.Resize(values, values, nullsVal.Rank(_key), nullsVal.Count + 1, -1);
        ...
        if( nullsVal[_key] )
        {
            values[IsFlat ?
                       _key :
                       nullsVal.Rank(_key) - 1] = value!;
            return false;
        }


        if( IsFlat ) values[_key] = value!;
        ...
        else
        {
            ...
            BitList.Resize(values,
                           nullsVal.Count < values.Length ?
                               values :
                               values = new V[values.Length * 2], r, nullsVal.Count, 1);
            values[r] = value!;
        }
    ...
    {
        if( !key.HasValue ) return keys._hasNullKey;
        var k = key.Value;
    ...
    public bool Remove(K? key)
    {
        ...
        var k    = key.Value;
        var _key = Unsafe.As<K, byte>(ref k);
        if( !keys.Set0(_key) ) return false;
    ...
    public struct Enumerator : IEnumerator<KeyValuePair<K?, V?>>{
        ...
        {
            if( _version != _map.keys._version ) throw new InvalidOperationException("Collection was modified during enumeration.");
            ...
            _current = new KeyValuePair<K?, V>(Unsafe.As<int, K>(ref _key), _map.nullsVal[_key] ?
                                                                                _map.values[_map.IsFlat ?
                                                                                                _key :
                                                                                                ++_valIndex] :
                                                                                default);
    ...
    private struct ValueEnumerator : IEnumerator<V?>{
        ...
        public bool MoveNext()
        {
            ...
            currentValue = _map.nullsVal[_key] ?
                               _map.values[_map.IsFlat ?
                                               _key :
                                               ++valIndex] :
                               default;
    ...
    private struct KeyEnumerator : IEnumerator<K?>{
        ...
        public K? Current => _version != _map.keys._version ?
                                 throw new InvalidOperationException("Collection was modified during enumeration.") :
                                 _key == int.MaxValue - 1 || _key == -2 || _key == -1 && !_map.HasNullKey ?
                                     throw new InvalidOperationException("Enumeration has either not started or has finished.") :
                                     _key == -1 ?
        ...
        public bool MoveNext()
        {
            ...
            if( ++_key == -1 || _key != int.MaxValue && (_key = _map.keys.Next1(_key)) != -1 ) return true;
            _key = int.MaxValue - 1;
            return false;
        }
    ...
    public override string ToString()
    {
        ...
        if( 1 < json.Length ) json.Length -= 1;
        json.Append('}');

...
public class NullablePrimitiveSet<K> : ISet<K?>, IReadOnlyCollection<K?> where K : unmanaged{
    ...
    public NullablePrimitiveSet(IEnumerable<K?> collection) : this(collection is ICollection<K?> coll ?
                                                                       coll.Count :
                                                                       0)
    {
    ...
    public bool Add(K? key)
    {
        ...
        if( _keys!.Length <= _loSize + _hiSize ) { Resize(_keys.Length * 2); }
        ...
        else
        {
            ...
            {
                if( EqualityComparer<K>.Default.Equals(_keys[i], kv) ) return false;
    ...
    public bool Contains(K? key)
    {
        ...
        {
            if( EqualityComparer<K>.Default.Equals(_keys![i], kv) ) return true;
    ...
    public bool Remove(K? key)
    {
        ...
        {
            if( !EqualityComparer<K>.Default.Equals(removeKey, kv) ) return false;
...
#region ISet Explicit/Remaining Implementation
    ...
    public void ExceptWith(IEnumerable<K?> other)
    {
        ...
        if( Count == 0 ) return;
        if( other is NullablePrimitiveSet<K> otherSet && otherSet.Count == 0 ) return;

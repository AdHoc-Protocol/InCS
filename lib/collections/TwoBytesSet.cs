...
/// </summary>
public class TwoBytesSet<K> : ISet<K>, IReadOnlySet<K> where K : unmanaged{
    ...
    public TwoBytesSet(IEnumerable<K> collection) : this(collection is ICollection<K> coll ?
                                                             coll.Count :
                                                             0)
    {
    ...
    public bool Contains(K key)
    {
        ...
        {
            if( toUshort(_keys![i]) == k_ushort ) return true;
    ...
    public bool Remove(K key)
    {
        ...
        else
        {
            ...
            if( toUshort(removeKey = ref _keys![removeIndex = link]) == kUshort ) // The key is found at 'SecondNode'
                if( removeIndex < _loSize ) link = _links![removeIndex];          // 'SecondNode' is in lo-region, relink to bypass it. `link` is a ref, so this modifies _links[last].
                else
                {
                    removeKey   = lastKey;
                ...
                for( var collisions = 0;; )
                {
                    ...
                    {
                        if( removeIndex < _loSize ) link = _links![removeIndex]; // `link` is a ref to the previous node's link, so this bypasses the current `removeIndex`.
                        else
                        {
                            removeKey   = lastKey;
                            removeKey   = ref lastKey;
...
#region ISet Explicit/Remaining Implementation
    ...
    public void ExceptWith(IEnumerable<K> other)
    {
        ...
        if( Count == 0 ) return;
        if( other is TwoBytesSet<K> otherSet && otherSet.Count == 0 ) return;

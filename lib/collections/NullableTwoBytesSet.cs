...
/// </summary>
public class NullableTwoBytesSet<K> : ISet<K?>, IReadOnlySet<K?> where K : unmanaged{
    ...
    public NullableTwoBytesSet(IEnumerable<K?> collection) : this(collection is ICollection<K?> coll ?
                                                                      coll.Count :
                                                                      0)
    {
    ...
    {
        foreach( var key in enumerable ) Add(key);
    }

    public int Count => (IsFlatStrategy ?
                             _flatCount :
                             _loSize + _hiSize) + (_hasNullKey ?
                                                       1 :
                                                       0);
    ...
    public bool Add(K? key)
    {
        ...
        while( true )
        {
            ...
            if( IsFlatStrategy )
            {
                ...
                var     t       = bitword;
                if( t == (bitword |= 1UL << i) ) return false;
                _flatCount++;
                _version++;
                return true;
            }
            ...
            if( _keys!.Length <= _loSize + _hiSize )
            {
                ...
                if( newSize > flatStrategyThreshold && _keys.Length < flatStrategyThreshold ) newSize = flatStrategyThreshold;
                Resize(newSize);
            ...
            else
            {
                ...
                {
                    if( toUshort(_keys[i]) == kUshort ) return false;
    ...
    public bool Contains(K? key)
    {
        ...
        var kv = key.Value;
        if( IsFlatStrategy ) return exists(kv);
        ...
        {
            if( toUshort(_keys![i]) == kUshort ) return true;
    ...
    public void CopyTo(K?[] dst, int dstIndex)
    {
        ...
        if( _hasNullKey ) dst[dstIndex++] = null;
        ...
        {
            // Copy lo-region
            for( var i = 0; i < _loSize; i++ ) dst[dstIndex++] = _keys[i];
            // Copy hi-region
            for( var i = _keys.Length - _hiSize; i < _keys.Length; i++ ) dst[dstIndex++] = _keys[i];
        }
    }
    ...
    private void Resize(int newSize)
    {
        ...
        if( flatStrategyThreshold < newSize )
        {
            _nulls = new ulong[NULLS_SIZE];
            // Copy lo-region
            for( var i = 0; i < _loSize; i++ ) exists1(_keys![i], _nulls);
            // Copy hi-region
            for( var i = _keys!.Length - _hiSize; i < _keys.Length; i++ ) exists1(_keys[i], _nulls);
        ...
        for( var i = 0; i                           < oldLoSize; i++ ) Copy(oldKeys![i]);
        for( var i = oldKeys!.Length - oldHiSize; i < oldKeys.Length; i++ ) Copy(oldKeys[i]);
    }
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    ...
    public bool Remove(K? key)
    {
        ...
        {
            if( toUshort(removeKey) != kUshort ) return false;
    ...
    public struct Enumerator : IEnumerator<K?>{
        ...
        {
            if( _version != _set._version ) throw new InvalidOperationException("Collection was modified during enumeration.");
            ...
            if( _index == int.MaxValue || (_set.IsFlatStrategy ?
                                               _set._flatCount :
                                               _set._loSize + _set._hiSize) == 0 )
            {
        ...
        public K? Current => _version != _set._version ?
                                 throw new InvalidOperationException("Collection was modified during enumeration.") :
                                 _index == int.MaxValue - 1 || _index == -2 || _index == -1 && !_set._hasNullKey ?
...
#region ISet Explicit/Remaining Implementation
    ...
    public void ExceptWith(IEnumerable<K?> other)
    {
        ...
        if( Count == 0 ) return;
        if( other is NullableTwoBytesSet<K> otherSet && otherSet.Count == 0 ) return;
        ...
        foreach( var key in other ) Remove(key);
    }
    ...
    public void UnionWith(IEnumerable<K?> other)
    {
        ...
        foreach( var key in other ) Add(key);
    }
    ...
    public bool IsSubsetOf(IEnumerable<K?> other)
    {
        ...
        if( IsFlatStrategy && other is NullableTwoBytesSet<K> { IsFlatStrategy: true } otherTbs )
        {
            if( _hasNullKey && !otherTbs._hasNullKey ) return false;
            for( var i = 0; i < NULLS_SIZE; i++ )
    ...
    {
        ArgumentNullException.ThrowIfNull(other);
        if( other is ICollection<K?> { Count: 0 } ) return true;

        // Fast path
        if( IsFlatStrategy && other is NullableTwoBytesSet<K> { IsFlatStrategy: true } otherTbs )
        {
            if( otherTbs._hasNullKey && !_hasNullKey ) return false;
            for( var i = 0; i < NULLS_SIZE; i++ )
    ...
    public bool Overlaps(IEnumerable<K?> other)
    {
        ...
        if( IsFlatStrategy && other is NullableTwoBytesSet<K> { IsFlatStrategy: true } otherTbs )
        {
            if( _hasNullKey && otherTbs._hasNullKey ) return true;
            for( var i = 0; i < NULLS_SIZE; i++ )
                if( (_nulls![i] & otherTbs._nulls![i]) != 0 )
                    return true;
    ...
    {
        ArgumentOutOfRangeException.ThrowIfNegative(capacity);
        var currentCapacity = _keys?.Length ?? (IsFlatStrategy ?
                                                    VALUES_SIZE :
                                                    0);
    ...
    public void TrimExcess()
    {
        ...
        else if( _keys != null )
        {
            ...
            if( newSize < _keys.Length ) { Resize(newSize); }
        }
    }
}